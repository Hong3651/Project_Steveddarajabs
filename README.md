...
서버가 실행되면 브라우저에서 `http://localhost:5000`으로 접속해 발표 영상을 업로드할 수 있습니다.

## 모델 파일 배치

추론 서버는 학습된 모델 가중치가 필요합니다. 모델 파일은 Git에 포함하지 않으며, 프로젝트 루트의 `모델집합/` 폴더에 아래 이름으로 배치합니다.
...
Code/
├── app.py                         # Flask 추론 서버
├── config.py                      # 경로, 모델, 학습/추론 설정
├── utils/
│   ├── datasets.py                # Dataset 및 오디오 feature extractor
│   ├── helpers.py                 # 데이터 준비, 학습, 추론, 후처리 유틸
│   └── models.py                  # Gesture, Audio, Text, Fusion 모델 정의
├── experiments/                   # 모델 비교 및 실험 노트북
└── outputs/                       # 실험 결과 HTML
```

자세한 포트폴리오/면접용 정리는 [`PROJECT_SUMMARY_KO.md`](PROJECT_SUMMARY_KO.md)를 참고하세요.
